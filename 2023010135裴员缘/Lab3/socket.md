...
| :--- | :------------------ | :--------------------- |
| `server.py` 已启动，`client.py` 尚未运行 |ss 输出中的 State（Windows 对应 netstat 状态）|阶段 1：创建套接字（监听阶段）|
| `client.py` 已连接，数据尚未发送完毕 |ESTABLISHED（Windows 对应 ESTABLISHED）|阶段 3：收发数据（通信阶段）|
| `server.py` 执行 `close()` 之后 |TIME_WAIT（连接消失前的等待状态）|阶段 4：断开连接|
...
| 项目 | 你的值 |
| :--- | :----- |
| `server` 套接字的描述符编号 |376|
| `conn` 套接字的描述符编号 |428/420 |
| 两者是否相同 |否 |
...
| :--- | :------- | :------- |
| 程序是否自动退出 |是|否（卡住） |
| `ss` 中连接的 State |TIME_WAIT（断开后）|ESTABLISHED（长连接保持）|
