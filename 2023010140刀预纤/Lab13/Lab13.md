...
| 网关 | 允许规则 | 日志前缀 | 默认策略 |
| :--- | :--- | :--- | :--- |
| `gwA` | 1. 放行 RELATED、ESTABLISHED 回程连接2. 仅允许源 10.1.0.0/24 访问目标 10.2.0.2 的 TCP 8080 新建流量|A-TO-B-DENY: | DROP|
| `gwB` | 1. 放行 RELATED、ESTABLISHED 回程连接2. 仅允许源 10.1.0.0/24 访问目标 10.2.0.2 的 TCP 8080 新建流量|VPN-S2S-DENY: | DROP|
...
| 测试 | 预期结果 | 实际结果 | 日志 |
| :--- | :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功 |成功，完整返回网页 HTML，服务端打印 200 访问日志 | 无 A-TO-B-DENY 拦截日志，流量匹配放行规则|
| `siteA -> siteB:2222` | 失败 |失败，curl 报错无法连接服务器 | 生成 A-TO-B-DENY 日志，记录 TCP DPT=2222 拦截报文|
| `siteA -> siteB ping` | 失败 | 失败，返回端口不可达，100% 丢包|生成多条 A-TO-B-DENY 日志，记录 ICMP TYPE=8 ping 拦截报文 |
...
| `latest handshake` |13 seconds ago |13 seconds ago |
...
| 测试 | 结果 | 命中的规则 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功| 放行 RELATED/ESTABLISHED + 允许 10.1.0.0/24 访问 10.2.0.2:8080 新建 TCP|
| `siteA -> siteB:2222` | 失败|无匹配放行规则，先 LOG (A-TO-B-DENY) 再 REJECT |
| `siteA -> siteB ping` | 失败|无匹配放行规则，先 LOG (A-TO-B-DENY) 再 REJECT |

### E. 抓包对比

| 抓包位置 | 看到的地址 | 协议 | 说明 |
| :--- | :--- | :--- | :--- |
| underlay | 192.0.2.1 ↔ 192.0.2.2|UDP 51820 |WireGuard 外层加密隧道报文，看不到内部业务 IP |
| `wg0` | 10.1.0.2 ↔ 10.2.0.2|TCP / HTTP |隧道内部明文原始业务流量，完整 TCP 握手 + HTTP |
| B 站点 LAN |10.1.0.2 ↔ 10.2.0.2 |TCP / HTTP | 服务器内网侧明文业务报文，和 wg0 流量一致|
...
答：远程接入：单终端接入内网，一对多；站点到站点：两个局域网双向互通，网对网。

2. 为什么站点到站点 VPN 中 `AllowedIPs` 应该写对端内网网段？

答：自动生成隧道路由，仅加密转发对端内网流量，限制流量范围，提升安全。

3. 为什么两个网关都必须开启 `net.ipv4.ip_forward=1`？

答：网关需完成内网网卡与隧道网卡三层转发，关闭则跨网段数据包直接丢弃。

4. 为什么两个站点内的普通主机都需要把默认网关指向本地 VPN 网关？

答：访问异地内网属于跨网段流量，必须交由网关转发进入 VPN 隧道。
...
答：好处：双向流量独立管控、分层日志、纵深防御；
仅单边配置不可以，反向访问会绕过安全策略

7. underlay 抓包看不到 `10.1.0.2` 和 `10.2.0.2`，这说明 VPN 隐藏了哪些信息？还暴露了哪些信息？

答：隐藏：内网私有 IP、业务端口、应用数据、内网拓扑；
暴露：两端公网地址、WireGuard 51820 UDP 端口、外层流量长度与时序。

8. 如果要允许 `siteB` 主动访问 `siteA` 的某个服务，需要增加哪些规则？

答：gwB 放行源 10.2.0.0/24 访问目标服务端口的新建流量；
gwA 放行隧道入站、目标服务端口的新建流量；
保留原有 RELATED、ESTABLISHED 回程放行规则。
