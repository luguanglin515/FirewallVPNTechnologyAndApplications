...
| `wan` 无法回复的原因 |	防火墙（fw）网络命名空间未配置回程路由，无法将 ICMP 响应数据包转发回 lan 网段，导致 ping 请求无法得到应答|
...
>答：（1）conntrack 额外记录信息：Lab8 中 conntrack 额外记录了 NAT 地址映射关系：内网真实源 IP、防火墙转换后的公网 IP、会话五元组信息，用于 NAT 回包时还原内网真实地址。
（2）说明的核心原理：防火墙状态检测、NAT 地址转换都依赖连接跟踪表（conntrack），conntrack 是实现状态防火墙与 NAT 功能的底层核心模块，统一管理网络连接状态与地址映射。

2. 如果两台内网主机同时访问同一个外网服务器的同一个端口，NAT 如何区分两条连接的回复分别属于哪台内网主机？
>答：NAT 通过 分配不同的防火墙侧临时源端口 区分两条连接：（1）两台内网主机访问同一外网 IP + 端口时，防火墙为每条连接分配唯一的对外源端口；2）外网服务器回复包的目的端口对应防火墙分配的临时端口；（3）防火墙查询 conntrack 表，根据该端口匹配到对应的内网主机，将数据包转发回去。
...
| TCP 38070 访问是否成功 |是 |
| ping 是否成功 |否 |
| ping 失败的原因 | 防火墙仅放行状态为 NEW、ESTABLISHED 的 TCP 38070 端口流量，未开放 ICMP（ping）协议，所有 ICMP 数据包被FORWARD DROP规则丢弃|
...
1. 为什么私有地址（如 `10.0.0.0/8`）的主机通常无法被互联网直接访问？NAT 是怎么解决这个问题的？
   ...
   > 答：（1）私有 IP 地址仅在内网有效，公网路由器不会转发私有网段的路由，互联网设备无法路由到私有地址；同时私有地址可在内网重复使用，公网无法唯一识别。（2）NAT（源地址转换）将内网私有 IP 替换为防火墙 / 路由器的公网 IP，内网主机对外统一使用公网 IP 通信；回包时 NAT 再将公网 IP 还原为内网私有 IP，实现内网主机访问互联网。


2. `MASQUERADE` 和 `SNAT --to-source 192.0.2.1` 在本实验中效果相同。实际场景中应如何选择？

   > 答：（1）SNAT（--to‑source）：适用于公网 IP 固定的场景（静态公网 IP），性能更好，直接指定固定转换后的 IP。（2）MASQUERADE：适用于公网 IP 动态变化的场景（如家用宽带拨号），自动获取出接口 IP 完成转换，无需手动指定地址。本实验中防火墙 WAN 口 IP 固定，因此两者效果一致。


3. NAT 对外隐藏了内网拓扑，但它给哪些应用层协议带来了麻烦？为什么？

   > 答：（1）主要影响主动对外提供服务、地址嵌入数据包载荷的协议，例如：FTP、SIP、P2P、IPsec、游戏联机等。（2）原因：这类协议会在数据包内容里携带内网私有 IP，NAT 只修改 IP 头部地址，无法修改载荷内的内网地址；外部设备收到后无法回连内网，导致通信失败。


4. 状态检测和 NAT 都依赖 conntrack，从这个角度看，为什么家用路由器通常把这两个功能集成在一起？

   > 答：（1）conntrack（连接跟踪表）统一记录连接五元组、状态、NAT 地址映射；（2）状态防火墙靠 conntrack 判断数据包是否属于合法连接；（3）NAT 靠 conntrack 记录内网 IP 与公网 IP / 端口的映射关系；（4）两者共享同一套连接跟踪表，集成在一起可减少重复开发、提升性能、降低成本，因此家用路由器将二者合并实现。

5. 任务五里 ping 被 FORWARD 链拦截。如果要在保留现有规则的基础上，让 ICMP 也能通过，应该怎么写规则？

   > 答：在原有DROP规则之前，添加放行 ICMP 协议的规则：sudo ip netns exec fw iptables -A FORWARD -p icmp -m conntrack --ctstate NEW,ESTABLISHED,RELATED -j ACCEPT 该规则放行 ICMP 新建、已建立、关联数据包，同时不改动原有 TCP 38070 的放行规则。
