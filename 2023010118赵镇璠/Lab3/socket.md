...
| :--- | :------------------ | :--------------------- |
| `server.py` 已启动，`client.py` 尚未运行 |LISTENING（LISTEN） | 监听（LISTEN）阶段|
| `client.py` 已连接，数据尚未发送完毕 |ESTABLISHED |连接建立（ESTABLISHED）阶段 |
| `server.py` 执行 `close()` 之后 |TIME_WAIT（或 FIN_WAIT_2 / CLOSE_WAIT，截图可见 TIME_WAIT、FIN_WAIT_2、CLOSE_WAIT） | 连接关闭（TIME_WAIT 等挥手阶段）|
...
| 项目 | 你的填写 |
| :--- | :------- |
| 连接的目标地址和端口 | 127.0.0.1:9999（截图未显示目标地址，但前面知道）|
...
（1）套接字：客户端的 socket() 已经成功创建，因此客户端套接字存在；但服务器未启动，服务器端的监听套接字不存在，无法响应连接请求。
（2）管道：TCP 连接需要三次握手才能建立，此时服务器未监听，握手失败，双向通信的管道完全不存在，没有建立任何有效连接。

2. `server.py` 打印出了两个不同的描述符编号（`server` 和 `conn`）。为什么 `accept()` 要返回一个新的套接字，而不是直接复用原来的 `server` 套接字？

   > 答：（1）原 server 套接字是监听套接字，仅负责在指定端口等待客户端的连接请求，不参与实际数据传输。accept() 返回的 conn 套接字是通信套接字，专门用于与已连接的客户端进行双向数据收发。
（2）复用原套接字会导致：无法同时处理多个客户端连接（监听和数据传输会互相阻塞），且无法区分不同客户端的连接。新套接字可以让监听套接字持续等待新连接，同时为每个已连接客户端分配独立的通信通道，实现并发处理。

3. 描述符和端口号都可以用来标识一个套接字，它们的本质区别是什么？各自解决了什么问题？

   > 答：描述符（文件描述符）是操作系统内核为进程分配的、仅在单个进程内部唯一的整数句柄，本质是进程访问内核中套接字对象的本地标识，作用范围局限于当前进程，不同进程的描述符可以重复，它解决的是进程如何快速、高效地定位和操作自己的套接字，实现对网络数据的读写；而端口号是 TCP/IP 网络层定义的、在整个主机全局唯一的 16 位整数标识，本质是网络层面区分不同网络应用的标识，作用范围覆盖整个网络，它解决的是操作系统如何将收到的网络数据，正确分发到主机上对应应用进程的问题，让一台主机可以同时运行多个网络服务、互不干扰。简单来说，描述符是进程内的 “本地编号”，管进程怎么用套接字；端口号是网络中的 “全局地址”，管数据怎么送到对应进程。


4. `client.py` 中，你有没有指定过客户端自己的端口号？这个端口号是谁分配的？在 `ss` 的输出中能看到它吗？

   > 答：（1）没有手动指定客户端端口号。
（2）客户端端口号由操作系统内核自动分配（称为 “临时端口 / 动态端口”），在调用 connect() 时自动分配。
（3）在 ss（或 netstat）的输出中可以看到，例如截图中 127.0.0.1:57518 这类客户端端口，就是操作系统分配的临时端口。

5. HTTP/1.0 实验中，即使你没有调用 `close()`，`recv()` 循环也会自动退出。解释这是为什么？

   > 答：HTTP/1.0 默认采用短连接模型：服务器在发送完完整的 HTTP 响应后，会主动调用 close() 关闭 TCP 连接。当服务器主动关闭连接时，客户端的 recv() 会收到EOF（文件结束符，即返回空字节 b''），此时 recv() 循环会自动退出，无需客户端手动调用 close()。

6. HTTP/1.1 实验中，程序卡在 `recv()` 不退出，`ss` 显示连接仍是 `ESTABLISHED`。这说明 HTTP/1.1 和 HTTP/1.0 在连接管理上有什么根本区别？

   > 答：两者的根本区别在于默认连接模型：
（1）HTTP/1.0：默认是短连接（非持久连接），服务器响应完成后主动关闭连接，客户端 recv() 收到 EOF 后自动退出。
（2）HTTP/1.1：默认是长连接（持久连接 /Connection: keep-alive），服务器不会主动关闭连接，而是保持 ESTABLISHED 状态，等待客户端发送新的 HTTP 请求。因此客户端 recv() 会一直阻塞等待数据，程序卡住，需要手动 Ctrl+C 终止。
