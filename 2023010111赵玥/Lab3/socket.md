...
| :--- | :------------------ | :--------------------- |
| `server.py` 已启动，`client.py` 尚未运行 |LISTENING |连接|
| `client.py` 已连接，数据尚未发送完毕 | ESTABLISHED| 收发数据|
| `server.py` 执行 `close()` 之后 |TIME-WAIT（最终消失）|断开 |
...
| :--- | :------- | :------- |
| 程序是否自动退出 |是 | 否|
| `ss` 中连接的 State |连接断开，无 ESTABLISHED | ESTABLISHED|
...
1. 实验中 `server.py` 必须比 `client.py` 先启动。如果顺序反过来，`client.py` 会报什么错误？用今天学到的概念解释：此时套接字存在吗？管道存在吗？
   ...
   > 答：client.py 会抛出 ConnectionRefusedError（连接被拒绝） 错误。此时客户端套接字存在，但因为服务器没有启动，没有建立 TCP 连接（管道不存在）。原因：服务器未进入 LISTEN 状态，客户端无法发起三次握手，连接被操作系统拒绝。


2. `server.py` 打印出了两个不同的描述符编号（`server` 和 `conn`）。为什么 `accept()` 要返回一个新的套接字，而不是直接复用原来的 `server` 套接字？

   > 答：原来的server套接字是监听套接字，它的唯一作用是持续监听端口，等待客户端发起连接请求，不能用来和客户端收发数据。accept()返回新的conn套接字，是为了给当前连接的客户端分配一个专属的通信套接字，专门负责和这个客户端收发数据。这样设计可以让server套接字继续监听新的连接，同时conn套接字处理已连接客户端的通信，实现服务器同时处理多个客户端连接的能力，因此不能复用原监听套接字。

3. 描述符和端口号都可以用来标识一个套接字，它们的本质区别是什么？各自解决了什么问题？

   > 答：本质区别：端口号是网络层面的标识，描述符是操作系统进程内部的标识，二者作用域完全不同。
端口号：解决的是「网络中数据要发送到主机的哪个服务」的问题，通过端口号区分同一台主机上的不同网络服务，是网络通信的寻址标识。
描述符：解决的是「进程内部要操作哪个套接字资源」的问题，是操作系统给进程内套接字分配的编号，让程序能通过描述符找到并操作对应的套接字。

4. `client.py` 中，你有没有指定过客户端自己的端口号？这个端口号是谁分配的？在 `ss` 的输出中能看到它吗？

   > 答：在client.py中没有指定客户端自己的端口号。客户端的端口号是操作系统自动分配的临时端口（动态端口），在ss（Windows 下为netstat）的输出中可以看到，表现为客户端侧的一个随机端口号（如之前截图中的53421这类随机端口）。

5. HTTP/1.0 实验中，即使你没有调用 `close()`，`recv()` 循环也会自动退出。解释这是为什么？

   > 答：HTTP/1.0 默认采用短连接设计，服务器在发送完完整的 HTTP 响应后，会主动关闭 TCP 连接。当服务器主动断开连接时，客户端的recv()会收到空字节数据，触发if not chunk: break的判断，因此recv()循环会自动退出，无需手动调用close()。

6. HTTP/1.1 实验中，程序卡在 `recv()` 不退出，`ss` 显示连接仍是 `ESTABLISHED`。这说明 HTTP/1.1 和 HTTP/1.0 在连接管理上有什么根本区别？

   > 答：二者的根本区别是默认连接模式不同：HTTP/1.0 默认是短连接：一次 HTTP 请求 / 响应完成后，服务器主动关闭 TCP 连接，因此recv()会收到空数据，循环自动退出，连接状态从ESTABLISHED消失。HTTP/1.1 默认是长连接（Keep-Alive）：服务器不会主动关闭 TCP 连接，连接持续保持ESTABLISHED状态，recv()会一直阻塞等待新的请求数据，因此程序卡住不退出，可复用同一连接发送多个 HTTP 请求。
