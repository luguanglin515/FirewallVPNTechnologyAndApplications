...
| :--- | :--- |
| ping 是否有回应 | 否 |
| `wan` 收到的请求源地址 | 192.0.2.2 |
| `wan` 无法回复的原因 | 没有回传路径，即wan的路由表中缺少到达lan网段的路由条目（且未经过NAT转换） |
...
1. 在 Lab7 中，conntrack 帮助防火墙判断"这个包属不属于合法连接"。在 Lab8 中，conntrack 还额外记录了什么信息？这两个功能都依赖 conntrack，说明了什么？
   > 在Lab8中，conntrack除了记录包的连接状态外，还额外记录了NAT前后的映射关系，即它保存了原始源地址与转换后源地址之间的对应条目。这两个功能都高度依赖conntrack，说明了连接跟踪表是现代状态检测防火墙和网络地址转换的核心逻辑中枢，它为无状态的IP协议提供了必要的“记忆”能力，使得安全访问控制和私有网段互联能够基于统一的会话上下文来运行。

2. 如果两台内网主机同时访问同一个外网服务器的同一个端口，NAT 如何区分两条连接的回复分别属于哪台内网主机？
   > 当多台内网主机同时访问同一外网目标时，NAT路由器会利用NAPT技术，为每一条不同的内网连接分配一个唯一的、不重复的临时源端口号。当外网服务器发回响应包时，路由器通过检查该包的“目的端口号”，在conntrack表中反向查找对应的内网IP和原始端口，从而精确地将回复包定向分发给正确的内网主机，避免了通信冲突。
...
| TCP 38070 访问是否成功 | 是 |
| ping 是否成功 | 否 |
| ping 失败的原因 | 当前`FORWARD`规则只允许TCP 38070的新连接 |
...
1. 为什么私有地址（如 `10.0.0.0/8`）的主机通常无法被互联网直接访问？NAT 是怎么解决这个问题的？
   ...
   > 答：私有地址在互联网路由协议中被规定为“非全球唯一”的，全球互联网路由器会丢弃所有目的地址为私有网段的数据包，导致外网设备找不到回程路径；NAT通过在网关处将内网主机的私有源IP动态替换为网关合法的公网IP，使得外网响应包能先发回网关，再由网关根据`conntrack`映射表将数据准确转发给内网主机。

2. `MASQUERADE` 和 `SNAT --to-source 192.0.2.1` 在本实验中效果相同。实际场景中应如何选择？

   > 答：`SNAT`适用于拥有固定、静态公网IP的服务器环境，因为它直接指定转换后的地址，处理效率更高；而`MASQUERADE`专门用于拨号上网或DHCP这种IP地址经常变化的动态环境，它会自动读取出口网卡的当前IP进行转换，能够避免因IP变动导致原有`SNAT`规则失效的问题。

3. NAT 对外隐藏了内网拓扑，但它给哪些应用层协议带来了麻烦？为什么？

   > 答：NAT会给诸如FTP、SIP或H.323等应用层协议带来麻烦，因为这些协议的载荷中往往包含主机真实的内网IP和端口信息；当NAT只修改了IP头部的地址而没能识别并修改应用层报文内的IP数据时，通信双方就无法建立后续的数据通道，通常需要ALG技术来辅助修正。

4. 状态检测和 NAT 都依赖 conntrack，从这个角度看，为什么家用路由器通常把这两个功能集成在一起？

   > 答：从技术底层看，状态检测防火墙和NAT共享同一套`conntrack`连接跟踪机制，集成在一起可以复用内存中的同一张状态表来同时完成安全审计和地址转换，这极大降低了硬件开销并提高了转发性能；从用户角度看，这种集成让内网主机在获得“访问外网”能力的同时，顺带获得了“防御外网主动入侵”的安全屏障，符合家用场景易用与安全的双重需求。

5. 任务五里 ping 被 FORWARD 链拦截。如果要在保留现有规则的基础上，让 ICMP 也能通过，应该怎么写规则？

   > 答：由于现有规则已经允许了`ESTABLISHED`状态的包，只需要针对请求方向补充一条放行规则即可，具体命令为：`iptables -A FORWARD -p icmp --icmp-type echo-request -m state --state NEW -j ACCEPT`；这样当Ping请求包被放行并建立连接记录后，回程的`Echo Reply`包就会自动命中已有的`ESTABLISHED`规则顺利通过。
