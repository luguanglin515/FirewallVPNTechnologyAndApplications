...
| :--- | :------------------ | :--------------------- |
| `server.py` 已启动，`client.py` 尚未运行 |LISTEN| 阶段1：创建套接字+阶段2：监听/等待连接|
| `client.py` 已连接，数据尚未发送完毕 |ESTABLISHED| 阶段3：正在通信（连接以建立但未完成数据交互）|
| `server.py` 执行 `close()` 之后 |（无连接/关闭状态）| 阶段4：断开连接|
...
1. 实验中 `server.py` 必须比 `client.py` 先启动。如果顺序反过来，`client.py` 会报什么错误？用今天学到的概念解释：此时套接字存在吗？管道存在吗？
   ...
   > 答：如果先运行 client.py，会出现连接被拒绝的错误。这是因为客户端发起连接请求时，服务器还没有启动，对应的端口没有处于监听状态，操作系统直接拒绝了连接。此时客户端的套接字已经创建成功，但服务器端没有对应的监听套接字，双方也没有建立任何通信管道，连接并未形成。

2. `server.py` 打印出了两个不同的描述符编号（`server` 和 `conn`）。为什么 `accept()` 要返回一个新的套接字，而不是直接复用原来的 `server` 套接字？

   > 答：服务器最初创建的套接字是专门用来监听客户端连接请求的，不负责收发数据。当客户端连接成功时，accept () 会返回一个新的通信套接字，专门用于和这个客户端进行数据交互。这样设计可以让监听套接字持续等待新的客户端连接，同时每个已连接的客户端都有独立的套接字处理通信，保证服务器可以同时处理多个客户端，不会出现连接和通信的冲突。

3. 描述符和端口号都可以用来标识一个套接字，它们的本质区别是什么？各自解决了什么问题？

   > 答：文件描述符是操作系统分配给当前进程的整数编号，只在当前进程内部有效，用来区分套接字、文件等不同的 I/O 对象。而端口号是网络协议中的标识，在整个网络和主机中有效，用来区分同一台主机上的不同网络应用。简单来说，文件描述符服务于进程内部管理，端口号服务于网络间的数据定位。

4. `client.py` 中，你有没有指定过客户端自己的端口号？这个端口号是谁分配的？在 `ss` 的输出中能看到它吗？

   > 答：在 client.py 中没有手动指定客户端的端口号。客户端使用的端口号是操作系统内核自动分配的临时端口，也叫动态端口，通常是 1024 以上的随机数字。在 ss 或 netstat 命令的输出结果中，可以清晰看到这个由系统自动分配的客户端端口号。

5. HTTP/1.0 实验中，即使你没有调用 `close()`，`recv()` 循环也会自动退出。解释这是为什么？

   > 答：HTTP/1.0 默认使用短连接，当服务器把响应数据发送完成后，会主动关闭 TCP 连接。连接关闭后，客户端的 recv () 会接收到空数据，程序判断到没有数据后就会自动退出循环，整个程序正常结束，不需要手动终止。

6. HTTP/1.1 实验中，程序卡在 `recv()` 不退出，`ss` 显示连接仍是 `ESTABLISHED`。这说明 HTTP/1.1 和 HTTP/1.0 在连接管理上有什么根本区别？

   > 答：HTTP/1.0 默认使用短连接，一次请求完成后服务器就会主动断开连接，客户端程序可以自动结束。HTTP/1.1 默认使用长连接，连接不会在一次请求后关闭，而是保持等待后续请求，因此客户端的 recv () 会一直等待数据，程序会卡住不退出，必须手动终止。两者最核心的区别就是连接管理方式，短连接每次请求都要重新建立连接，长连接可以复用同一个连接处理多次请求，效率更高。
