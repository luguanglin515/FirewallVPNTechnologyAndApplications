...
| :--- | :------------------ | :--------------------- |
| `server.py` 已启动，`client.py` 尚未运行 |LISTENING |服务器启动并监听端口，等待连接 |
| `client.py` 已连接，数据尚未发送完毕 |ESTABLISHED | TCP 连接已成功建立，正在保持连接|
| `server.py` 执行 `close()` 之后 |TIME_WAIT |连接主动关闭后，等待套接字资源回收 |
...
1. 实验中 `server.py` 必须比 `client.py` 先启动。如果顺序反过来，`client.py` 会报什么错误？用今天学到的概念解释：此时套接字存在吗？管道存在吗？
   ...
   > 答：报错信息：客户端会抛出 ConnectionRefusedError（Windows 下为 WinError 10061，目标计算机主动拒绝连接）。
套接字存在吗：存在。客户端调用 socket() 时，操作系统会为其创建一个本地套接字（分配文件描述符），只是该套接字无法完成 connect() 三次握手，连接失败。
管道存在吗：不存在。TCP 全双工管道的建立必须完成三次握手，服务器未监听端口时，SYN 包无响应，连接无法建立，因此不存在端到端的通信管道。

2. `server.py` 打印出了两个不同的描述符编号（`server` 和 `conn`）。为什么 `accept()` 要返回一个新的套接字，而不是直接复用原来的 `server` 套接字？

   > 答：server 套接字是监听套接字，仅负责监听端口、接受客户端连接请求，不参与具体数据传输。
conn 套接字是连接套接字，由 accept() 生成，专门用于与已连接的客户端进行双向数据收发。
复用原套接字会导致：同一套接字同时处理新连接请求和已有连接的数据传输，造成逻辑混乱、并发阻塞。新套接字实现了监听与数据传输的解耦，支持服务器同时处理多个客户端连接。

3. 描述符和端口号都可以用来标识一个套接字，它们的本质区别是什么？各自解决了什么问题？

   > 答：本质区别：
描述符是操作系统内核为进程分配的本地文件句柄，仅在当前进程内唯一，属于操作系统层面的标识；端口号是 TCP/IP 协议栈定义的16 位网络标识，用于在全局网络中唯一标识一台主机上的一个进程，属于网络协议层面的标识。
各自解决的问题：
描述符让进程能通过操作系统内核管理套接字，实现send/recv等 I/O 操作，解决了进程与内核之间的套接字管理问题；端口号让一台主机能同时运行多个网络服务，区分不同应用的网络数据，解决了网络通信中多进程的寻址问题。

4. `client.py` 中，你有没有指定过客户端自己的端口号？这个端口号是谁分配的？在 `ss` 的输出中能看到它吗？

   > 答：是否指定：没有。客户端代码中仅指定了服务器的 IP:端口，未指定自身端口。
分配主体：由操作系统内核自动分配一个临时端口（又称 “ephemeral port”，通常为 1024~65535 区间的随机端口）。
是否可见：能看到。在 ss/netstat 输出中，ESTABLISHED 状态的本地端口即为客户端的临时端口（如 127.0.0.1:52224 中的 52224）。

5. HTTP/1.0 实验中，即使你没有调用 `close()`，`recv()` 循环也会自动退出。解释这是为什么？

   > 答：HTTP/1.0 默认采用短连接（Connection: close），服务器返回完整响应后，会主动发送 FIN 包关闭 TCP 连接。客户端 recv() 读取到服务器发送的 FIN 包时，会返回空字节串 b''，触发循环退出条件，因此无需手动调用 close()，程序自动退出。

6. HTTP/1.1 实验中，程序卡在 `recv()` 不退出，`ss` 显示连接仍是 `ESTABLISHED`。这说明 HTTP/1.1 和 HTTP/1.0 在连接管理上有什么根本区别？

   > 答：HTTP/1.0：默认短连接，服务器响应后主动关闭连接，客户端 recv() 自动退出，连接状态变为 TIME_WAIT。
HTTP/1.1：默认长连接（Keep-Alive），服务器响应后不主动关闭连接，保持 ESTABLISHED 状态，等待客户端后续请求。
根本区别：HTTP/1.0 是 “一次请求 - 一次连接”，HTTP/1.1 是 “多次请求 - 一次连接”，长连接大幅减少了 TCP 握手 / 挥手的开销，提升了性能。
