...
2. 状态码 `404` 代表什么含义？状态码 `500` 和 `503` 有什么区别？
   ...
   > 答：404 表示资源不存在。500 是服务器内部错误，代码异常；503 是服务不可用，服务器过载或维护。

3. GET 与 POST 方法的主要区别是什么？各适用于什么场景？

   > 答：GET 参数在 URL 中，有长度限制，可缓存，用于查询数据；POST 参数在请求体中，无长度限制，不可缓存，用于提交数据。

4. HTTP 与 HTTPS 有什么区别？HTTPS 使用了什么机制来保护数据？

   > 答：HTTP 明文传输，不安全；HTTPS 加密传输，安全。HTTPS 使用SSL/TLS 加密、身份认证、数据完整性校验机制。

5. 既然 HTTPS 已经加密，为什么浏览器开发者工具仍然能看到请求和响应的明文内容？

   > 答：因为浏览器是通信的一端，HTTPS 是浏览器与服务器之间加密，开发者工具在浏览器内部，能拿到解密后的数据，所以可见明文。
