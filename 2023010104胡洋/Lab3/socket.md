...
| :--- | :------------------ | :--------------------- |
| `server.py` 已启动，`client.py` 尚未运行 |listen |服务端启动监听阶段 |
| `client.py` 已连接，数据尚未发送完毕 |established | 建立连接,客户端连接成功|
| `server.py` 执行 `close()` 之后 |time-wait |服务端关闭连接 |
...
1. 实验中 `server.py` 必须比 `client.py` 先启动。如果顺序反过来，`client.py` 会报什么错误？用今天学到的概念解释：此时套接字存在吗？管道存在吗？
   ...
   > 答：报连接拒绝错误。本机8080端口无服务监听，客户端请求被系统拦截。此时没有客户端套接字，因为connect失败未创建；也没有对应的管道/连接，服务端未监听，无连接通道。

2. `server.py` 打印出了两个不同的描述符编号（`server` 和 `conn`）。为什么 `accept()` 要返回一个新的套接字，而不是直接复用原来的 `server` 套接字？

   > 答：server套接字负责监听新连接，conn 负责与当前客户端通信。复用会导致监听与数据传输逻辑冲突，无法同时处理新连接和旧连接的数据。

3. 描述符和端口号都可以用来标识一个套接字，它们的本质区别是什么？各自解决了什么问题？

   > 答：描述符是进程内的文件句柄，标识进程内的套接字实例，解决进程内资源管理问题；端口号是网络层的逻辑标识，标识主机上的服务进程，解决网络中不同服务的区分与数据寻址问题。

4. `client.py` 中，你有没有指定过客户端自己的端口号？这个端口号是谁分配的？在 `ss` 的输出中能看到它吗？

   > 答：未指定。客户端端口由操作系统随机分配。在netstat 输出的本地地址列中可以看到该端口。

5. HTTP/1.0 实验中，即使你没有调用 `close()`，`recv()` 循环也会自动退出。解释这是为什么？

   > 答：HTTP/1.0 默认短连接，服务器响应完数据后主动关闭 TCP 连接，触发客户端recv返回空数据，循环自动退出。

6. HTTP/1.1 实验中，程序卡在 `recv()` 不退出，`ss` 显示连接仍是 `ESTABLISHED`。这说明 HTTP/1.1 和 HTTP/1.0 在连接管理上有什么根本区别？

   > 答：HTTP/1.1 默认长连接，服务器响应完不主动关闭连接，保持ESTABLISHED状态；HTTP/1.0 默认短连接，响应完主动关连。
