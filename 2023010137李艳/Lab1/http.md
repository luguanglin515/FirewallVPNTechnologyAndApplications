...
| :----------------- | :------------- |
| 请求方法（Method） |get|
| 请求路径（URI）    |https://www.kmust.edu.cn/|
| 协议版本           |HTTP/1.1|
| Host               |www.kmust.edu.cn|
...
2. 状态码 `404` 代表什么含义？状态码 `500` 和 `503` 有什么区别？
   ...
   > 答：404表示资源未找到，状态码：NOT FOUND。500是服务器内部错误，503是服务暂时不可用，，500是是服务器故障性问题，503是服务器临时性问题。


3. GET 与 POST 方法的主要区别是什么？各适用于什么场景？

   > 答：GET：数据在URL，大小受限，安全性底，可缓存，用于获取资源。POST：数据在请求体，大小无限制，安全性高，不缓存，用于提交数据。

4. HTTP 与 HTTPS 有什么区别？HTTPS 使用了什么机制来保护数据？

   > 答：HTTP：明文传输，端口80，无证书。HTTPS：加密传输，端口443，需要SSL/TLS证书；通过SSL/TLS协议加密、验证身份。

5. 既然 HTTPS 已经加密，为什么浏览器开发者工具仍然能看到请求和响应的明文内容？

   > 答：加密在网络传输阶段，浏览器已解密数据，开发者工具展示解密后的内容。
