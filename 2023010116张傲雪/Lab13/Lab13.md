...
| :--- | :--- | :--- |
| `gwA` |10.2.0.0/24,10.100.0.2/32 |10.2.0.0/24 dev wg0   10.1.0.0/24 dev wg0 |
| `gwB` |10.1.0.0/24,10.100.0.1/32 |10.1.0.0/24 dev wg0 |
...
| 网关 | 允许规则 | 日志前缀 | 默认策略 |
| :--- | :--- | :--- | :--- |
| `gwA` |ACEEPT,REJECT,LOG |A-TO-B-DENY: |FORWARD DROP |
| `gwB` |ACEEPT,REJECT,LOG|VPN-S2S-DENY: |FORWARD DROP |
...
| 测试 | 预期结果 | 实际结果 | 日志 |
| :--- | :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功 |成功 |无日志（流量被放行，不触发 LOG 规则） |
| `siteA -> siteB:2222` | 失败 |失败 |存在多条A-TO-B-DENY TCP 拦截日志 |
| `siteA -> siteB ping` | 失败 |失败 |存在多条A-TO-B-DENY ICMP 拦截日志 |
...
| `latest handshake` |18 seconds ago|18 seconds ago |
...
| 测试 | 结果 | 命中的规则 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` |成功 |ACCEPT允许 |
| `siteA -> siteB:2222` |失败 |LOG 日志规则 + REJECT 拒绝规则 |
| `siteA -> siteB ping` |不可达|LOG 日志规则 + REJECT 拒绝规则 |
...
| `wg0` |10.1.0.2 ↔ 10.2.0.2 |TCP 8080 |解密后原始业务内网流量 |
| B 站点 LAN |10.1.0.2 ↔ 10.2.0.2 |TCP 8080 |防火墙放行后转发至内网服务器的明文业务包 |
...
答：远程接入：单个移动客户端访问企业总部内网，一端是单人设备，一端是网关。
站点到站点：两个完整私有局域网互通，两端都是站点网关，各自承载多台内网主机。
2. 为什么站点到站点 VPN 中 `AllowedIPs` 应该写对端内网网段？
答：WireGuard 的 AllowedIPs 有两大核心作用：
路由下发：内核自动为配置的网段生成dev wg0路由，访问对端内网的流量会自动送入 WireGuard 隧道封装；安全校验：校验隧道解封装后的内层数据包源 IP，若内层源地址不在 peer 的 AllowedIPs 范围内，直接丢弃数据包，防止隧道侧 IP 欺骗攻击。
站点到站点场景需要整段内网互通，因此直接填写对端完整内网网段；只写隧道单点 IP 只能实现网关之间互通，无法让内网主机跨站访问。
3. 为什么两个网关都必须开启 `net.ipv4.ip_forward=1`？
答：网关同时连接内网 veth 网卡和WireGuard 隧道 wg0 网卡，属于三层转发设备：gwA 需要转发：siteA 内网流量 → wg0 隧道；以及隧道回程流量 → siteA 内网；gwB 需要转发：wg0 隧道入站流量 → siteB 内网；以及 siteB 回程流量 → wg0 隧道；IP 转发关闭时，操作系统只会处理本机接收的数据包，不会在两个网卡之间转发流量，跨站点访问会直接中断。
4. 为什么两个站点内的普通主机都需要把默认网关指向本地 VPN 网关？
答：内网主机没有 wg0 隧道接口，无法识别对端10.2.0.0/24/10.1.0.0/24路由：主机访问跨站点网段时，没有匹配路由，会匹配默认路由，将数据包发给本地网关 gwA/gwB；网关拥有指向对端网段的 wg0 路由，接收主机数据包后送入 VPN 隧道转发；若不配置默认网关，主机访问异地网段会提示Network unreachable，无法发起通信。
5. `wg show` 里有握手但业务不通，排查顺序应该是什么？
答：按从底层到上层、从网络到防火墙的顺序排查：检查两端网关ip_forward=1是否开启；核对ip route，确认对端内网网段路由存在且出接口为 wg0；检查内网主机默认网关是否正确指向本地网关；抓包 wg0 接口，确认数据包是否完成封装 / 解封装；检查 iptables FORWARD 链默认策略与拦截规则；核对 WireGuard 两端 AllowedIPs 是否包含完整对端内网网段；检查 MTU 是否匹配（WireGuard 标准 MTU 1420，防止分片丢包）。
6. 本实验中 `gwA` 和 `gwB` 都写防火墙规则，有什么好处？只在一边写规则可以吗？
答：好处：双向规则确保了通信的双向性，避免单向规则导致的连接问题。只在一边写规则会导致对端无法主动访问，通信会受限。
单边可行，但存在缺陷，仅 gwA 配置：非法流量仍会加密传输经过 underlay 网络到达 gwB，浪费隧道带宽，且 gwB 无日志审计；一旦 gwA 防火墙失效，siteB 内网完全暴露。
仅 gwB 配置：所有内网访问流量全部通过隧道传输，大量无效加密数据包占用网络资源，无法在源头拦截。
7. underlay 抓包看不到 `10.1.0.2` 和 `10.2.0.2`，这说明 VPN 隐藏了哪些信息？还暴露了哪些信息？
答：被 WireGuard 加密隐藏的内容
原始内网源 / 目的 IP（10.1.0.2、10.2.0.2 等私有网段）；上层业务协议（TCP/UDP、目标端口 8080/2222、ICMP ping 报文）；应用层 HTTP 请求数据、数据包载荷
外层 Underlay 暴露的信息
两端公网 / 底层互联 IP（192.0.2.1、192.0.2.2）；WireGuard 固定 UDP 端口 51820；数据包传输时间、包长度、流量收发总量；存在 VPN 通信行为，但无法识别通信内容与访问目标。
8. 如果要允许 `siteB` 主动访问 `siteA` 的某个服务，需要增加哪些规则？
答： WireGuard 配置无需修改现有 AllowedIPs 已经双向放行完整内网网，段，路由天然互通。
gwB 出站防火墙新增规则，在 gwB FORWARD 链新增允许 siteB 主动发起访问 siteA 服务的 NEW 连接规则，放在 LOG、REJECT 规则之前。
 gwA 入站防火墙新增放行规则，在 gwA FORWARD 链隧道入站段，允许隧道过来的 siteB 流量访问 siteA 服务。
