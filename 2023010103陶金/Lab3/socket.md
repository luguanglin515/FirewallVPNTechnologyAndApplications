...
| :--- | :------------------ | :--------------------- |
| `server.py` 已启动，`client.py` 尚未运行 | `LISTEN` | 监听阶段（等待连接） |
| `client.py` 已连接，数据尚未发送完毕 | `ESTAB` | 数据传输阶段（连接已建立） |
| `server.py` 执行 `close()` 之后 | `TIME-WAIT` | 连接终止阶段（挥手阶段） |
...
| 是你调用 `close()` 触发的断开，还是服务器主动断开的 | `close()`触发的断开 |
...
1. 实验中 `server.py` 必须比 `client.py` 先启动。如果顺序反过来，`client.py` 会报什么错误？用今天学到的概念解释：此时套接字存在吗？管道存在吗？
   ...
   > 答：Windows会报`ConnectionRefusedError: [WinError 10061] 由于目标计算机积极拒绝，无法连接。`Linux会报`ConnectionRefusedError: [Errno 111] Connection refused`
   客户端的套接字存在但关闭，服务端的套接字不存在；TCP的管道建立在成功连接的基础上，所以不存在。

2. `server.py` 打印出了两个不同的描述符编号（`server` 和 `conn`）。为什么 `accept()` 要返回一个新的套接字，而不是直接复用原来的 `server` 套接字？

   > 答：`server`用于监听是否有新的客户端发起连接请求，`conn`专门负责与特定的客户端进行数据收发，可以实现高并发，如果仅有一个套接字只能一对一进行服务。

3. 描述符和端口号都可以用来标识一个套接字，它们的本质区别是什么？各自解决了什么问题？

   > 答：描述符仅对当前进程可见，随进程结束而消失，端口号对全网可见，随套接字释放而空闲，可被其他进程复用；描述符解决了“进程如何操作资源”的问题，端口号解决了“数据包该交给谁”的问题。

4. `client.py` 中，你有没有指定过客户端自己的端口号？这个端口号是谁分配的？在 `ss` 的输出中能看到它吗？

   > 答：没有，这个端口号是由操作系统内核动态分配的；可以看到。

5. HTTP/1.0 实验中，即使你没有调用 `close()`，`recv()` 循环也会自动退出。解释这是为什么？

   > 答：在HTTP/1.0协议中，默认的操作模式是短连接，服务器在处理完一个请求并发送完响应报文后，必须主动关闭TCP连接。

6. HTTP/1.1 实验中，程序卡在 `recv()` 不退出，`ss` 显示连接仍是 `ESTABLISHED`。这说明 HTTP/1.1 和 HTTP/1.0 在连接管理上有什么根本区别？

   > 答：在HTTP/1.1协议中，默认的操作模式是长连接，即使数据传输完成，TCP连接依然维持。二者对TCP的操作不同，导致使用HTTP/1.1协议程序卡在 `recv()` 不退出。
