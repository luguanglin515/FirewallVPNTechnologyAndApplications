...
| :--- | :------------------ | :--------------------- |
| `server.py` 已启动，`client.py` 尚未运行 |LISTEN（Windows 显示 LISTENING）|服务器监听阶段（等待连接）|
| `client.py` 已连接，数据尚未发送完毕 |ESTAB（Windows 显示 ESTABLISHED）|连接建立、数据收发阶段|
| `server.py` 执行 `close()` 之后 |TIME-WAIT（Windows 显示 TIME_WAIT）|连接关闭、资源回收阶段|
...
| :--- | :------- | :------- |
| 程序是否自动退出 |是|否|
| `ss` 中连接的 State |连接断开（无 ESTABLISHED，或短暂 TIME_WAIT 后消失）|ESTABLISHED（保持连接）|
...
1. 实验中 `server.py` 必须比 `client.py` 先启动。如果顺序反过来，`client.py` 会报什么错误？用今天学到的概念解释：此时套接字存在吗？管道存在吗？
   ...
   > 答：client.py 会抛出 ConnectionRefusedError（连接被拒绝）错误。客户端套接字存在：socket() 已成功创建，只是未完成连接。管道不存在：server.py 未启动，8080 端口无 LISTENING 监听套接字，TCP 三次握手无法完成，不存在端到端的通信管道。

2. `server.py` 打印出了两个不同的描述符编号（`server` 和 `conn`）。为什么 `accept()` 要返回一个新的套接字，而不是直接复用原来的 `server` 套接字？

   > 答：server 套接字是监听套接字，仅负责在指定端口监听连接请求，不参与数据收发。accept() 返回的 conn 是通信套接字，专门用于与已连接的客户端进行数据收发。若复用 server 套接字，服务器将无法同时处理多个客户端连接（会造成连接混淆），因此必须为每个新连接创建独立的通信套接字，实现多连接并发管理。

3. 描述符和端口号都可以用来标识一个套接字，它们的本质区别是什么？各自解决了什么问题？

   > 答：端口号用于网络标识，让外部程序找到服务；描述符是系统内部标识，管理每个打开的套接字。端口解决 “如何被访问”，描述符解决 “系统如何区分资源”。

4. `client.py` 中，你有没有指定过客户端自己的端口号？这个端口号是谁分配的？在 `ss` 的输出中能看到它吗？

   > 答：没有指定客户端端口，端口由操作系统自动随机分配。在输出中可以看到客户端端口。

5. HTTP/1.0 实验中，即使你没有调用 `close()`，`recv()` 循环也会自动退出。解释这是为什么？

   > 答：因为 HTTP/1.0 服务器发送完响应后会主动关闭连接，recv() 收到空数据，循环自动退出。

6. HTTP/1.1 实验中，程序卡在 `recv()` 不退出，`ss` 显示连接仍是 `ESTABLISHED`。这说明 HTTP/1.1 和 HTTP/1.0 在连接管理上有什么根本区别？

   > 答：说明 HTTP/1.1 默认使用长连接（保持不断开），而 HTTP/1.0 是短连接（收发完立即断开）。
