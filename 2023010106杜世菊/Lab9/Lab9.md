...
| 测试项 | 结果（成功/失败） | 现象描述 |
| :----- | :-------------- | :------- |
| `office -> dmz:8080` |成功|正常返回网页目录列表，连接访问正常|
| `guest -> dmz:8080` |失败|连接被拒绝，curl 提示无法建立连接|
| `guest -> office` |失败|连接被拒绝，curl 提示无法建立连接|
...
> 答：不会被命中。原因：iptables 规则是按顺序从上到下匹配的。当数据包命中 REJECT 规则后，会直接被拒绝，规则匹配流程会立即终止，后面的 LOG 规则就永远没有机会被执行了。因此，必须将 LOG 规则放在 REJECT 之前，才能保证数据包被拦截前先被记录日志。
...
| :--- | :------- | :---------- | :------- |
| `guest -> dmz:8080` |失败（Connection refused）|是|GUEST-TO-DMZ:|
| `guest -> office` |	失败（Connection refused）|是|GUEST-TO-OFFICE:|
| `office -> dmz:8080` |成功（返回 HTML 目录）|否|-|

**从终端 D 复制一条实际日志行：**

```text
May 26 22:57:42 localhost kernel: GUEST-TO-DMZ: IN=veth-fw-guest OUT=veth-fw-dmz MAC=b2:60:11:60:95:db:7e:48:65:4e:c7:ce:08:00 SRC=10.0.2.2 DST=10.0.3.2 LEN=60 TOS=0x00 PREC=0x00 TTL=63 ID=26502 DF PROTO=TCP SPT=60692 DPT=8080 WINDOW=64240 RES=0x00 SYN URGP=0
```
...
> 答：合理。本实验中我们只对 被 REJECT 的违规访问 配置了 LOG 规则，而对 ACCEPT 的正常业务流量没有添加日志规则，因此成功访问不会产生日志，这是实验设计的预期行为。如果想记录 “所有被放行的流量”，需要在对应的 ACCEPT 规则之前，添加一条相同匹配条件的 LOG 规则，例如：sudo ip netns exec fw iptables -I FORWARD 2 -s 10.0.1.0/24 -d 10.0.3.2 -p tcp --dport 8080 -j LOG --log-prefix "OFFICE-TO-DMZ: " --log-level 4

2. 从日志里你能读出什么信息？"谁在访问谁的什么服务"能还原出来吗？

> 答：可以还原，日志中包含了完整的访问信息：
SRC=10.0.2.2：访问的源 IP（来自 guest 区域）
DST=10.0.3.2：访问的目的 IP（目标是 dmz 区域）
DPT=8080：访问的目标端口（Web 服务端口）
PROTO=TCP：使用的传输层协议
FLAGS=SYN：表示这是一个 TCP 连接请求包
通过这些字段，可以明确还原出：guest 区域的主机（10.0.2.2）尝试访问 dmz 区域（10.0.3.2）的 8080 端口 TCP 服务。
---
...
> 答：快速筛选事件：从海量系统日志中，精准过滤出特定类型的防火墙事件（如区分 GUEST-TO-DMZ 和 GUEST-TO-OFFICE）。
便于统计分析：可结合 wc -l 等工具，直接统计各类违规访问的频次，用于安全审计和趋势分析。
简化故障排查：在发生安全事件时，能快速定位攻击来源、目标和方式，大幅缩短问题排查时间。
支持告警与自动化：标准化的前缀日志便于对接监控系统，实现异常访问的实时告警。

2. 如果防火墙每秒拦截上千个包，不加限制地写 LOG 会有什么后果？

> 答：日志风暴：日志文件会快速膨胀，在短时间内占用大量磁盘空间，甚至导致磁盘被写满。
系统性能下降：频繁的日志写入会拉高系统的 CPU、内存和 I/O 负载，影响防火墙的转发性能和系统整体稳定性。
关键事件被淹没：大量重复的拦截日志会淹没真正的关键安全事件（如首次攻击、异常扫描），不利于安全审计和故障排查。
运维成本增加：需要额外的资源来处理、存储和分析海量日志，增加了运维复杂度和成本。
...
| 触发访问次数 | 10 |
| 实际写入日志条数 |≤5|
| `--limit-burst 5` 的含义 |允许初始突发最多记录 5 条日志，之后按 --limit 3/min 的速率限制写入频率|
...
> 答：二者是相互独立、互不影响的关系：
LOG 仅作为日志记录模块，只负责将数据包信息写入内核日志，不改变数据包的转发状态，也不终止规则链的匹配流程。
REJECT 是真正的访问控制模块，会直接拒绝数据包并终止规则链，决定访问是否被拦截。
即使 LOG 因限流未记录日志，后续的 REJECT 规则依然会对所有数据包生效，安全策略不会因为日志限制而失效。

2. "没记录就当没发生"和"仍然拦截但不记录"有本质区别吗？为什么安全审计中这个区别很重要？

> 答：二者有本质区别：
“没记录就当没发生” 代表访问被直接放行，安全策略未起作用，数据包成功进入目标主机，存在安全风险。
“仍然拦截但不记录” 代表访问被安全策略成功拦截，只是出于性能考虑，通过 --limit 精简了日志记录，并未降低防护等级。
在安全审计中，这个区别至关重要：
可区分 “策略放行、策略拦截、日志限流” 三种状态，避免误判为安全事件未发生。
防止因日志限流而错误评估防护效果，明确安全策略始终在执行，只是日志输出被优化。
便于追溯和分析，即使日志不全，也能通过规则命中计数（如 iptables -L -v 的 pkts 字段）验证拦截行为，保证审计的完整性和准确性。
...
| 测试项 | 结果 |
| :----- | :--- |
| `office -> dmz:8080` |成功，返回 HTML 目录列表|
| `guest -> dmz:8080` |失败，提示 Could not connect to server|
| `guest -> office` |失败，提示 Could not connect to server|
...
| :------- | :----- | :------- | :---------- |
| 1 |ACCEPT|所有 ESTABLISHED,RELATED 状态的连接|无|
| 2 |ACCEPT|源 10.0.1.0/24、目的 10.0.3.2、TCP 协议、目的端口 8080|无|
| 3 |LOG|源 10.0.2.0/24、目的 10.0.3.2、TCP 协议、目的端口 8080|	GUEST-TO-DMZ:|
| 4 |	REJECT|源 10.0.2.0/24、目的 10.0.3.2、TCP 协议、目的端口 8080|无|
| 5 |LOG|源 10.0.2.0/24、目的 10.0.1.0/24、所有协议|GUEST-TO-OFFICE:|
| 6 |REJECT|源 10.0.2.0/24、目的 10.0.1.0/24、所有协议|无|
...
1. 为什么 `dmz` 的服务不应该和 `office` 混在同一个网段？用本实验的规则效果来说明。
   ...
   > 答：DMZ 区域的服务对外暴露，安全风险更高。如果和办公网同网段，一旦访客区域的主机突破 DMZ 服务，就能直接访问办公网主机，本实验中 guest 被限制无法访问 office，正是通过网段隔离实现的；若不隔离，防火墙规则无法区分服务和办公主机，访客可直接访问办公网数据，导致内网安全失效。

2. 为什么 `guest` 一般应该和办公网隔离？

   > 答：访客区域的主机安全可信度低，可能存在恶意软件或攻击行为。与办公网隔离后，可防止访客主机通过网络漏洞非法访问办公终端、窃取业务数据或发起内网攻击，保护办公环境的安全性。

3. 第一阶段中为什么必须保留 `ESTABLISHED,RELATED` 规则？如果去掉会怎样？

   > 答：TCP 连接是双向的，office 发起访问 dmz 的请求后，服务端的响应报文需要通过 FORWARD 链返回。保留该规则可允许合法连接的响应报文通行；若去掉，响应报文会被默认的 DROP 策略拦截，导致 office 发起的所有访问都无法成功建立连接。

4. `LOG` target 和 `ACCEPT`/`DROP`/`REJECT` 最本质的区别是什么？为什么 LOG 必须放在 REJECT 前面？

   > 答：本质区别：LOG 仅将数据包信息写入内核日志，不改变数据包的转发状态，也不终止规则链匹配；而 ACCEPT/DROP/REJECT 会直接决定数据包的命运（放行或拦截），并终止规则链匹配。
   LOG 必须放在 REJECT 前面，因为 REJECT 会直接终止规则链，若 LOG 放在后面则永远不会被执行，无法记录被拦截的访问日志。

5. `office -> dmz` 的成功访问没有日志，被拦截的 `guest` 访问有日志。这是合理的还是疏漏？为什么？

   > 答：这是合理的。本实验的日志规则仅配置在拦截访问的规则前，目的是审计违规访问行为，正常业务流量无需日志记录，可减少日志冗余，符合 “仅记录异常事件” 的安全审计原则。


6. `--limit` 控制日志频率后，超出限制的包仍然被 REJECT 但不再写入日志。这和"只要不记录就当没发生"有本质区别吗？

   > 答：有本质区别。“只要不记录就当没发生” 意味着访问被直接放行，安全策略未起作用；而 --limit 仅限制了日志写入频率，数据包依然会被 REJECT 规则拦截，安全防护策略始终生效，只是出于性能考虑精简了日志输出。

7. 如果后面要接入 VPN，你认为 VPN 用户应该先接入哪个区域？结合网络分区和日志审计两个角度回答。

   > 答：应先接入 guest（访客）区域。
   网络分区角度：VPN 用户属于外部接入的用户，安全可信度与访客主机一致，接入访客区域可通过现有规则限制其访问办公网和 DMZ 的权限，避免直接暴露内网。
   日志审计角度：可利用已有的 GUEST-TO-* 日志规则，对 VPN 用户的访问行为进行统一记录和审计，便于后续追溯违规访问行为。
