...
| 测试 | 预期结果 | 你的结果 |
| :--- | :--- | :--- |
| `siteA -> 10.1.0.1` | 成功 | 成功，0% 丢包|
| `siteB -> 10.2.0.1` | 成功 | 成功，0% 丢包|
| `gwA -> 192.0.2.2` | 成功 |成功，0% 丢包 |
| `siteA -> 10.2.0.2` | 失败 |不可达 / 100% 丢包 |
...
| 测试 | 预期 | 你的结果 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功 | 成功，返回文件目录 HTML|
| `siteA -> siteB:2222` | 成功 | 成功，返回文件目录 HTML|
...
| 网关 | 允许规则 | 日志前缀 | 默认策略 |
| :--- | :--- | :--- | :--- |
| `gwA` |内网 10.1.0.0/24 访问 10.2.0.2 TCP 8080 新建连接 | A-TO-B-DENY| FORWARD DROP|
| `gwB` |隧道源 10.1.0.0/24 访问 10.2.0.2 TCP 8080 新建连接 |VPN-S2S-DENY |FORWARD DROP |
...
| 测试 | 预期结果 | 实际结果 | 日志 |
| :--- | :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功 |正常访问页面 | 无拒绝日志|
| `siteA -> siteB:2222` | 失败 | curl 连接超时失败	| 产生 A-TO-B-DENY 日志|
| `siteA -> siteB ping` | 失败 | ICMP 目的端口不可达	| 产生 A-TO-B-DENY 日志|
...
| `latest handshake` |几秒前，握手正常 |几秒前，握手正常 |
| `transfer` 计数 |收发数据包字节非 0 | 收发数据包字节非 0|
...
| 测试 | 结果 | 命中的规则 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` |成功 |gwA/gwB 8080 新建连接允许规则 |
| `siteA -> siteB:2222` |失败 |gwA LOG 记录规则、gwA REJECT 拒绝规则 |
| `siteA -> siteB ping` | 失败| gwA LOG 记录规则、gwA REJECT 拒绝规则|

### E. 抓包对比

| 抓包位置 | 看到的地址 | 协议 | 说明 |
| :--- | :--- | :--- | :--- |
| underlay |192.0.2.1 <-> 192.0.2.2 |UDP 51820	 |底层加密外层 UDP 报文 |
| `wg0` | 10.1.0.2 <-> 10.2.0.2| TCP 8080| 隧道解密后的真实内网流量|
| B 站点 LAN |10.1.0.2 <-> 10.2.0.2 |TCP 8080	 |放行后转发至服务器的业务包 |
...
远程接入 VPN：一端是单个远程客户端，另一端是企业内网；客户端 AllowedIPs 填写总部内网，网关仅放行客户端隧道 IP，满足单人远程办公。
站点到站点 VPN：两端均为完整私有内网网段，网关互填对方完整内网 CIDR，实现两个办公网段双向互通，适用于总部 - 分支、多机房互联。
2. 为什么站点到站点 VPN 中 `AllowedIPs` 应该写对端内网网段？
AllowedIPs 兼具路由与安全校验双重作用：
① 路由层面：wg-quick 自动生成对应网段路由，目标为对端内网的流量自动送入 wg0 隧道封装；
② 安全校验：隧道收到数据包时，强制校验内层源 IP 匹配 AllowedIPs，阻止非法内网 IP 通过隧道渗透。
站点场景需要整段网段互通，因此填写对方内网大网段。
3. 为什么两个网关都必须开启 `net.ipv4.ip_forward=1`？
gwA、gwB 同时连接内网 LAN 与隧道 wg0，属于三层转发设备。关闭转发时内核仅接收本机流量，不会跨接口转发报文；开启后才能完成内网↔隧道之间的数据转发。
4. 为什么两个站点内的普通主机都需要把默认网关指向本地 VPN 网关？
siteA 访问 10.2.0.2 不属于本地直连网段，若无默认路由则无转发路径；全部跨站点流量交给 gwA，由 gwA 通过 WireGuard 隧道路由转发至对端站点。
5. `wg show` 里有握手但业务不通，排查顺序应该是什么？
1）核对两端 AllowedIPs 是否填写完整对端内网网段；
2）检查 gwA/gwB 是否开启 ip_forward 转发；
3）确认 siteA/siteB 主机默认路由指向本地网关；
4）检查 iptables FORWARD 链是否拦截跨站点流量；
5）在 wg0 接口抓包，确认解密内网报文正常转发；
6）检查 underlay 网络是否放行 UDP 51820 端口。
6. 本实验中 `gwA` 和 `gwB` 都写防火墙规则，有什么好处？只在一边写规则可以吗？
优势：纵深防御。gwA 出口拦截非法流量，减少无效加密传输；gwB 入口二次防护，即使 gwA 配置出错，分支内网仍受保护；双端日志可分别审计出站、入站非法访问。
仅单边可以连通，但存在缺陷：只配 gwA 则非法流量仍会加密占用隧道带宽；只配 gwB 会让内网非法流量全部发往公网，存在安全隐患，不推荐生产使用。
7. underlay 抓包看不到 `10.1.0.2` 和 `10.2.0.2`，这说明 VPN 隐藏了哪些信息？还暴露了哪些信息？
隐藏信息：内网真实 IP（10.1.0.2、10.2.0.2）、业务端口、TCP 会话、应用明文数据。
暴露信息：underlay 公网 IP、WireGuard 固定 UDP 51820 端口、数据包长度、流量收发时间（可被流量特征分析）。
8. 如果要允许 `siteB` 主动访问 `siteA` 的某个服务，需要增加哪些规则？
1）gwB 新增规则：允许 veth-gwb-lan→wg0、源 10.2.0.0/24、目标 siteA 服务 IP + 端口的 NEW TCP 流量，配套 LOG、REJECT 兜底；
2）gwA 新增规则：允许 wg0→veth-gwa-lan、源 10.2.0.0/24、目标 siteA 服务 IP + 端口的 NEW TCP 流量；
3）保留两端 ESTABLISHED,RELATED 回程放行规则，保证双向应答；
4）现有 WireGuard 配置无需修改，双向网段路由已存在。
---
