...
| 失败的直接原因                   |路由表中没有去 10.20.0.0/24 的条目，包不知道走哪个接口，无法发出|

> 简答：为什么 `wg show` 显示隧道还在，但业务访问已经失败？隧道握手和业务数据传输用的是两套不同的路径还是同一套？
答：隧道握手和业务数据传输用的是两套不同的路径。握手走的是底层 underlay 地址（192.0.2.1:51820），与 AllowedIPs 无关；而业务数据（curl）的出口路由由 AllowedIPs 决定。当 AllowedIPs 改成 10.30.0.0/24 后，去 10.20.0.2 的包不再匹配路由表，所以根本不会进入 wg0 隧道，即使底层隧道是完好的，业务也无法访问。
截图：
...
| ------------------------ | ---------------------------------- | -------- |
| 看到的源地址             |192.0.2.2（client underlay）  192.0.2.1（vpn underlay）|10.10.10.1（client wg0）10.20.0.2（server）|
| 看到的目的地址           |192.0.2.1（vpn underlay）192.0.2.2（client underlay）|10.20.0.2（server）10.10.10.1（client wg0）|
| 协议                     |UDP|ICMP|
| 是否能看到 ICMP/HTTP 内容 |否|是|
| 为什么内外两层看到的不同 |外层是加密封装的 UDP 包，只有 VPN 端点地址；内层是解密后的原始 IP 包，包含真实通信内容。WireGuard 在发送前把整个内层 IP 包加密，然后装入 UDP 负载中，所以外层看不到内层信息。|外层是加密封装的 UDP 包，只有 VPN 端点地址；内层是解密后的原始 IP 包，包含真实通信内容。WireGuard 在发送前把整个内层 IP 包加密，然后装入 UDP 负载中，所以外层看不到内层信息。|

> 简答：如果把 VPN 隧道比喻成"信封+信纸"，outer（underlay）抓到的信息对应什么？inner（wg0）抓到的信息对应什么？这样设计对安全性意味着什么？
答：outer（underlay）抓到的信息对应信封：信封上写的是寄件人地址（192.0.2.2）和收件人地址（192.0.2.1），以及信封类型（UDP 51820）。沿途的人只能看到信封，不知道里面装的是什么。inner（wg0）抓到的信息对应信纸：信纸上写的是真实的通信内容（10.10.10.1 发 ICMP 给 10.20.0.2）。只有收信人拆开信封（解密）后才能看到。
截图：
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：作用一（出口路由）：wg-quick up 会把 AllowedIPs 里的网段写入系统路由表，告诉操作系统"发往这些地址的包走 wg0 接口"；作用二（入口白名单）：当收到解密后的内层包时，检查包的内层源地址是否在该 peer 的 AllowedIPs 范围内，不在则丢弃；为什么首先影响路由：因为 wg-quick 会自动将 AllowedIPs 添加到路由表，包必须首先匹配路由才能进入隧道，否则连 wg0 都到不了。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：握手用的是底层 underlay 地址（192.0.2.1:51820），与 AllowedIPs 无关，只要 Endpoint 可达就能完成；业务数据传输依赖 AllowedIPs 生成的路由，当 AllowedIPs 不包含目标网段时，包根本不会进入 wg0 隧道。所以握手和数据走的是"两条路"，互不依赖。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：客户端的所有流量（包括访问互联网、内网、甚至本地网络）都会被迫走 VPN 隧道，可能造成：访问本地网络也要绕路、流量延迟增加、VPN 服务器看到用户的所有流量、攻击者如果控制了 VPN 服务器，可以截获用户的所有通信。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：不能。 WireGuard 使用公钥认证，双方握手时需要互相验证对方身份：client 连接 vpn 时，client 用自己的私钥签名，vpn 用 client 的公钥验证，攻击者没有 client 私钥，无法伪造 client 的身份签名，握手会失败。这和 SSH 一样：只有公钥不够，必须有对应的私钥才能登录。

5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：外层源地址是 client 的 underlay 地址 192.0.2.2。抓包者看不到内层真实内容，因为 WireGuard 把内层 IP 包（包括源/目的地址和数据）整体加密后放入 UDP 负载中，未经解密无法读取。



6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：机密性：原始通信内容（ICMP/HTTP）被加密，中间人无法读取。通信关系隐藏：内层的真实源和目的地址（10.10.10.1 → 10.20.0.2）被封装，中间人只能看到两个 VPN 节点在通信，不知道具体是谁在访问谁
