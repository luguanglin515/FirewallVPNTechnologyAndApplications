...
| :--- | :------- | :------- |
| 程序是否自动退出 |是|否|
| `ss` 中连接的 State |CLOSE_WAIT → LAST_ACK → 连接关闭的过程|ESTABLISHED|
...
1. 实验中 `server.py` 必须比 `client.py` 先启动。如果顺序反过来，`client.py` 会报什么错误？用今天学到的概念解释：此时套接字存在吗？管道存在吗？
   ...
   > 答：会报 ConnectionRefusedError: [WinError 10061] 由于目标计算机积极拒绝，无法连接的 错误。此时服务器的监听套接字不存在，因为 server.py 还未创建并绑定端口。管道也不存在，因为 TCP 连接需要经过三次握手，而服务器没有处于 LISTEN 状态的套接字来响应客户端的 SYN 请求。

2. `server.py` 打印出了两个不同的描述符编号（`server` 和 `conn`）。为什么 `accept()` 要返回一个新的套接字，而不是直接复用原来的 `server` 套接字？

   > 答：server套接字专门用于监听连接请求，而 accept()返回的新套接字（conn）用于与客户端进行数据传输。这样设计可以实现一个服务器同时处理多个客户端连接，监听套接字持续接收新连接，每个连接有独立的套接字进行数据交换。

3. 描述符和端口号都可以用来标识一个套接字，它们的本质区别是什么？各自解决了什么问题？

   > 答：描述符是进程内部的整数标识符，由操作系统分配给进程，用于进程内唯一标识打开的文件或套接字。端口号是网络层面的标识符，用于在网络中定位特定主机上的特定服务。描述符解决了进程如何管理和访问套接字的问题；端口号解决了网络数据包如何路由到正确应用程序的问题。

4. `client.py` 中，你有没有指定过客户端自己的端口号？这个端口号是谁分配的？在 `ss` 的输出中能看到它吗？

   > 答：没有指定客户端端口号。客户端端口号由操作系统内核自动分配（通常从临时端口范围中选择）。在输出中可以看到，例如截图中的 62582 就是客户端端口号。

5. HTTP/1.0 实验中，即使你没有调用 `close()`，`recv()` 循环也会自动退出。解释这是为什么？

   > 答：HTTP/1.0 默认使用短连接，服务器在发送完响应数据后会主动关闭连接。当服务器关闭连接时，客户端的 recv() 会返回空字符串，循环条件 while data: 会判断为 False，从而退出循环。

6. HTTP/1.1 实验中，程序卡在 `recv()` 不退出，`ss` 显示连接仍是 `ESTABLISHED`。这说明 HTTP/1.1 和 HTTP/1.0 在连接管理上有什么根本区别？

   > 答：HTTP/1.1 默认使用持久连接（Keep-Alive），服务器发送完响应后不会立即关闭连接，而是保持连接以等待后续请求。客户端无法通过连接关闭来判断响应结束，因此 recv() 会一直阻塞等待更多数据。这与 HTTP/1.0 的短连接模式（每次请求后关闭连接）形成根本区别。
