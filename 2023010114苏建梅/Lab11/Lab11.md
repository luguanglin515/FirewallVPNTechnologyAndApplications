...
| `ip route` 中 wg0 相关的条目 | 10.10.10.0/24 dev wg0 proto kernel scope link src 10.10.10.1
10.20.0.0/24 dev wg0 scope link         |
...
| 失败的直接原因                   | 	client 路由表中没有去往 10.20.0.0/24 的路由，发往 10.20.0.2 的包无法被送到 wg0 接口         |

> 简答：为什么 `wg show` 显示隧道还在，但业务访问已经失败？隧道握手和业务数据传输用的是两套不同的路径还是同一套？
答：`wg show` 显示的是隧道本身的握手和保活状态，这部分使用的是对端的 Endpoint 地址（192.0.2.1:51820），跟 AllowedIPs 无关。而业务访问需要依靠 AllowedIPs 生成的路由把数据包送进隧道。当 AllowedIPs 从 10.20.0.0/24 改成 10.30.0.0/24 后，去往 10.20.0.2 的路由消失了，curl 的包根本进不了 wg0 接口，所以业务失败。但隧道底层基于 UDP 的会话仍然正常，握手和保活照常进行，所以 wg show 显示隧道还在。隧道握手和业务数据传输用的是同一套物理隧道（同一个 wg0 接口，同一个 UDP 连接），区别在于握手不经过路由表直接走 Endpoint，而业务数据必须先匹配路由规则才能进入隧道。
截图：
...
| 看到的目的地址           |  192.0.2.1                                  |      10.10.10.2    |
| 协议                     |    UDP                                |    ICMP      |
| 是否能看到 ICMP/HTTP 内容 |      否，只看到十六进制字节                              |  是，直接显示 ICMP echo request/reply        |
| 为什么内外两层看到的不同 |              underlay 接口上看到的是 WireGuard 封装后的外层 UDP 包，原始 ICMP 报文被加密后作为 UDP 载荷传输；                      |    wg0 接口是 WireGuard 虚拟接口，显示的是解密后的内层原始报文。      |

> 简答：如果把 VPN 隧道比喻成"信封+信纸"，outer（underlay）抓到的信息对应什么？inner（wg0）抓到的信息对应什么？这样设计对安全性意味着什么？
答：如果把 VPN 隧道比喻成信封加信纸，那么 underlay 抓到的信息对应信封上的内容，包括寄件人地址（192.0.2.2）、收件人地址（192.0.2.1）以及信封类型（UDP）。wg0 抓到的信息对应信封里面的信纸，上面写的才是真正要传达的内容，比如寄给 10.20.0.2 的 ICMP 请求或 HTTP 请求。这样设计对安全性的意义在于，当有人在网络传输路径上截获这个数据包时，只能看到两个 WireGuard 节点在通信，却看不到通信双方真正的 IP 地址，也看不到传输的具体内容是什么协议、请求了什么资源。这种双重保护既隐藏了通信关系，又隐藏了通信内容。
截图：
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：`AllowedIPs` 做了两件事：第一，控制本端路由——告诉操作系统哪些目标 IP 地址的流量应该通过这个 WireGuard 隧道发送；第二，控制对端访问权限——当对端发来数据包时，WireGuard 会检查包的内层源地址是否在 `AllowedIPs` 范围内，只有匹配的包才会被接受。之所以说它“看起来像访问控制，实际上首先影响的是路由”，是因为大多数用户第一次看到 `AllowedIPs` 时，会直观地认为它是在限制“谁可以连接过来”（即防火墙式的访问控制）。但在本端，它首先被 `wg-quick` 用来添加路由规则，决定哪些流量被送入隧道。路由是数据包能否进入隧道的前提条件——没有匹配的路由，包根本到不了 WireGuard 接口，对端的访问控制也就无从谈起。所以它的第一重作用是路由，第二重作用才是访问控制。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：在本实验中，`wg show` 显示隧道在线但 `curl` 失败，根本原因在于隧道握手和业务数据传输依赖两个不同的机制，它们可以独立工作。隧道握手依赖的是配置文件中 `[Peer]` 部分的 `Endpoint` 地址（即 underlay IP 192.0.2.1:51820）。WireGuard 使用这个固定的 IP 地址和端口与对端建立 UDP 会话、发送握手包和保活包。只要 underlay 网络连通，`Endpoint` 可达，握手就能成功，隧道状态就会显示 `latest handshake`。业务数据传输依赖的是路由表。当 `AllowedIPs` 被改成 `10.30.0.0/24` 后，`wg-quick` 自动删除了去往 `10.20.0.0/24` 的路由。`curl 10.20.0.2` 发出的包在系统路由表中找不到匹配条目，因此永远无法被送入 `wg0` 接口。既然包根本进入不了隧道，WireGuard 也就没有机会对它们进行加密和发送。简单说：隧道握手走的是 underlay 的 `Endpoint` 地址，不依赖路由表；业务数据走的是路由表，由 `AllowedIPs` 派生出的路由规则决定。两者共享同一条物理隧道，但在入口条件上是独立的：握手有 `Endpoint` 就够了，业务数据必须先有路由才能进隧道。所以隧道可以“活着”，但业务“过不去”。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：如果将 `AllowedIPs` 写成 `0.0.0.0/0`，意味着本端所有发往任意 IPv4 地址的流量都会被送入 WireGuard 隧道，依赖对端进行路由转发。这样做的风险取决于对端的信任程度和配置。如果对端是一个不可信或已被攻破的节点，本端的所有流量（包括 DNS 查询、Web 浏览、SSH 连接等）都会明文暴露给对端（隧道加密仅保护传输过程，对端可以看到解密后的内容）。对端可以记录、分析甚至篡改这些流量。如果对端是一个内部网络的 VPN 网关，但本端并不希望将所有互联网流量都经过该网关（例如访问本地局域网打印机或本机服务），`0.0.0.0/0` 会导致这些本不应出隧道的流量也被错误地送走，造成访问失败或产生不必要的网络延迟和带宽消耗。此外，`AllowedIPs` 还具有访问控制的作用。将对端的 `AllowedIPs` 设为 `0.0.0.0/0` 意味着接受该对端发来的任何源地址的内层包，攻击者如果能够控制该对端或伪造其身份，可能利用这一点绕过本地的源地址检查，发起 IP 欺骗攻击。因此，通常推荐将 `AllowedIPs` 限制为真正需要通过隧道的子网范围，而不是使用 `0.0.0.0/0`。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：不能。WireGuard 的握手需要双方互相验证对方的公钥。当 client 发起连接时，client 会用自己的私钥对数据进行签名，vpn 节点收到后用 client 的公钥验证签名。同时 vpn 节点也会用自己的私钥签名响应，client 用 vpn 的公钥验证。如果攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他无法伪装成 client 向 vpn 发起握手，因为他没有 client 的私钥来完成 client 端的签名。反过来，他可以伪装成 vpn 节点，但因为缺少 client 的私钥，他无法解密 client 发来的数据，也无法以 client 的身份向其他节点发起连接。所以单方面持有 vpn 私钥只能被动监听或尝试中间人攻击，但无法成功模拟 client 的身份建立隧道。

5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：根据截图中的抓包结果，底层抓包看到的外层源地址是 `192.0.2.2`（client 的 underlay 地址），目的地址是 `192.0.2.1`（vpn 的 underlay 地址）。抓包者看不到内层真实通信内容（如 ICMP echo request、HTTP GET）的原因有两个：第一，WireGuard 在内层 IP 包（原始 ICMP 或 HTTP 报文）外面封装了一层 UDP 头和外层 IP 头。抓包者在 underlay 接口上只能看到最外层的封装信息，内层报文被当作 UDP 的载荷传输。第二，WireGuard 会对整个内层 IP 包进行加密。即便抓包者看到 UDP 载荷里的十六进制内容，这些内容也是加密后的密文，无法还原出原始的 ICMP 或 HTTP 报文。因此，抓包者只能知道两个 WireGuard 节点（192.0.2.2 和 192.0.2.1）之间在传输 UDP 数据，但不知道传输的具体内容是什么协议、什么地址、什么数据。

6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：任务四中内外两层抓包看到的信息完全不同，说明了 VPN 封装机制提供的两方面安全保障。第一，身份与拓扑隐藏。外层抓包只能看到 underlay 地址 192.0.2.2 和 192.0.2.1，而内层真实的通信双方地址 10.10.10.1 和 10.20.0.2 在外层不可见。攻击者无法从抓包中推断出内部网络的拓扑结构、通信双方的真实 IP 以及他们之间的访问关系。第二，内容加密。外层抓包只能看到 UDP 头和加密后的十六进制载荷，无法还原出内层的 ICMP 或 HTTP 明文。即使攻击者截获了数据包，也无法读取传输的具体内容，比如请求了什么资源、执行了什么命令。
