...
| :--- | :------------------ | :--------------------- |
| `server.py` 已启动，`client.py` 尚未运行 |LISTEN |阶段 1（创建套接字）+ 服务器侧监听准备（阶段 2 前置） |
| `client.py` 已连接，数据尚未发送完毕 |ESTABLISHED | 阶段 2（连接完成）+ 阶段 3（收发数据）|
| `server.py` 执行 `close()` 之后 | CLOSED/TIME_WAIT|阶段 4（断开） |
...
| :--- | :------- | :------- |
| 程序是否自动退出 | 是| 否|
| `ss` 中连接的 State |ESTABLISHED（通信中）→ CLOSED（通信后） |持续 ESTABLISHED（长连接保持） |
...
1. 实验中 `server.py` 必须比 `client.py` 先启动。如果顺序反过来，`client.py` 会报什么错误？用今天学到的概念解释：此时套接字存在吗？管道存在吗？
   ...
   > 答：会报错 Connection refused (连接拒绝)。此时客户端套接字已创建，但服务器没有监听，通信管道不存在，无法建立连接。

2. `server.py` 打印出了两个不同的描述符编号（`server` 和 `conn`）。为什么 `accept()` 要返回一个新的套接字，而不是直接复用原来的 `server` 套接字？

   > 答：server 套接字专门负责监听连接请求；新 conn 套接字专门负责和客户端收发数据，职责分离，一个服务器可以同时处理多个客户端连接。

3. 描述符和端口号都可以用来标识一个套接字，它们的本质区别是什么？各自解决了什么问题？

   > 答：端口号是网络标识，让外部设备找到服务；描述符是操作系统内部标识，让进程管理套接字。端口号解决 "网络上怎么找到你"，描述符解决 "系统内部怎么管理你"。


4. `client.py` 中，你有没有指定过客户端自己的端口号？这个端口号是谁分配的？在 `ss` 的输出中能看到它吗？

   > 答：没有手动指定，操作系统自动分配临时端口；在 ss 命令输出中可以看到客户端的临时端口号。

5. HTTP/1.0 实验中，即使你没有调用 `close()`，`recv()` 循环也会自动退出。解释这是为什么？

   > 答：因为 HTTP/1.0 默认服务器发送完数据就主动断开连接，recv () 收到空数据就自动退出循环。

6. HTTP/1.1 实验中，程序卡在 `recv()` 不退出，`ss` 显示连接仍是 `ESTABLISHED`。这说明 HTTP/1.1 和 HTTP/1.0 在连接管理上有什么根本区别？

   > 答：HTTP/1.1 默认开启长连接 (keep-alive)，连接不会自动断开，所以程序卡在 recv () 等待数据，连接一直保持 ESTABLISHED。
